...
module {
  ...
  public class TestQuestions() = {
    ...
    let array_modified_ : [Question] = [
      ...
      { id = 2; author = Principal.fromText("zl5om-yevaq-syyny-vn5bl-ahjnu-cc2qx-b7oqi-ojbct-xrxjw-ivql6-uqe"); title = "A good citizen is a patriot.";               text = ""; date = 2432; status = #VOTING( { stage = #CATEGORIZATION; iteration = Iteration.openCategorization(Iteration.new(0), 2432, []) });                  interests_history = []; vote_history = []; },
      { id = 3; author = Principal.fromText("ytsdx-ddotz-rkcxu-mfivi-nvtwo-cv5ip-uw5jh-7om6u-gano3-ev6sl-3qe"); title = "The labor market enslaves workers.";         text = ""; date = 1321; status = #VOTING( { stage = #CATEGORIZATION; iteration = Iteration.openCategorization(Iteration.new(0), 1321, []) });                  interests_history = []; vote_history = []; },
      { id = 4; author = Principal.fromText("zzzno-jyjub-5bu5a-nnvpt-w52zs-chfkz-bd4ar-ztjzy-xjz24-i4r3g-gae"); title = "We should be retiring earlier.";             text = ""; date = 7234; status = #VOTING( { stage = #OPINION; iteration = Iteration.new(7234); });                                                             interests_history = []; vote_history = []; },
      { id = 5; author = Principal.fromText("lejdd-efwn5-h3qqe-4bunw-faabt-qwb7j-oiskz-c3dkg-3q5z5-ozrtn-dqe"); title = "Marriage should be abolished.";              text = ""; date = 3132; status = #VOTING( { stage = #OPINION; iteration = Iteration.new(3132); });                                                             interests_history = []; vote_history = []; },
      ...
      { id = 7; author = Principal.fromText("gbvlf-igtmq-g5vs2-skrhr-txgij-4f2j3-v2jqy-re5cm-i6hsu-gpzcd-aae"); title = "We must fight against global warming.";      text = ""; date = 3421; status = #VOTING( { stage = #CATEGORIZATION; iteration = Iteration.openCategorization(Iteration.new(0), 4213, []) });                  interests_history = []; vote_history = []; },
      { id = 8; author = Principal.fromText("mrdr7-aufxf-oiq6j-hyib2-rxb5m-cqrnb-uzgyq-durnt-75u4x-rrvow-iae"); title = "Exploitation of fossil fuels is necessary."; text = ""; date = 5431; status = #VOTING( { stage = #OPINION; iteration = Iteration.new(5431); });                                                             interests_history = []; vote_history = []; },
      { id = 9; author = Principal.fromText("zoyw4-o2dcy-xajcf-e2nvu-436rg-ghrbs-35bzk-nakpb-mvs7t-x4byt-nqe"); title = "The police should be armed.";                text = ""; date = 9711; status = #VOTING( { stage = #OPINION; iteration = Iteration.new(9711); });                                                             interests_history = []; vote_history = []; },
    ];
    ...
    public func getSuite() : Suite.Suite {
      ...
      let iter_interest = queries.entries(#STATUS(#INTEREST), #fwd);
