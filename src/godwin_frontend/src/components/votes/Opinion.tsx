...
const VoteOpinion = ({question_id}: Props) => {
  ...
  const updateOpinion = async () => {
    ...
    <div className="flex flex-col items-center space-x-1">
      ...
      <input id="small-range" min="-1" max="1" step="0.1" disabled={!logged_in} type="range" onChange={(e) => setOpinion(Number(e.target.value))} onMouseUp={(e) => updateOpinion()} className="w-24 h-1 mb-6 bg-gray-200 rounded-lg appearance-none cursor-pointer range-sm dark:bg-gray-700"></input>
    </div>
	);
};
