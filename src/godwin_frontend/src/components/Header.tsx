...
const Header = () => {
  ...
  const {isAuthenticated, authClient, btcBalance, gwcBalance, loggedUserName} = useContext(ActorContext);
		...
		<>
      {
        ...
        <header className="bg-slate-100 dark:bg-gray-800 sticky top-0 z-30 flex flex-row items-center w-full justify-between space-x-2 xl:px-4 lg:px-3 md:px-2 px-2 xl:h-18 lg:h-16 md:h-14 h-14">
          <Link to="/" className="flex flex-row items-center space-x-1">
            <span className="text-4xl">{ CONSTANTS.LOGO }</span>
          ...
          <div className="flex flex-row items-center justify-end md:space-x-4 space-x-2">
            ...
            <div className="block text-black dark:text-white">
              <Balance amount={ btcBalance !== null ? btcBalance : undefined } ledger_type={LedgerType.BTC} allow_conversion={true} default_unit={LedgerUnit.ORIGINAL}/>
            </div>
            <div className="block text-black dark:text-white">
              <Balance amount={ gwcBalance !== null ? gwcBalance : undefined } ledger_type={LedgerType.GWC} allow_conversion={true} default_unit={LedgerUnit.ORIGINAL}/>
            </div>
