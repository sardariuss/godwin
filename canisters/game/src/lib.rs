...
//      query, params).await;
//   let json_data : Value = serde_json::from_str(&json_str.0).unwrap();
//   let users : [User] = serde_json::from_value(json_data["data"]["readUser"].clone()).unwrap();
//   return users;
//}
...
async fn add_question(author_id: String, text: String) -> Question{
   let query = r#"mutation($author_id: ID!, $text: String!) {
      createQuestion(
        input: {author: {connect: $author_id}, text: $text, interest: 0}
      ) {
        id
        author {id username}
        text
...
async fn get_question(question_id: String) -> Question {
   let query = r#"query($question_id:ID!){
